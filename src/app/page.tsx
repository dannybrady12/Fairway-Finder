    ...
    <div className="max-w-5xl mx-auto px-4 py-10 space-y-10">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {rounds.map((round) => (
              <div key={round.id} className="border rounded-lg p-4 bg-gray-50">
                <h3 className="text-lg font-bold">{round.course?.name}</h3>
                <p className="text-sm text-gray-500">{round.course?.city}, {round.course?.state}</p>
                <p className="text-sm mt-1">Score: <strong>{round.total_score || 'N/A'}</strong></p>
                <p className="text-xs text-gray-400 mt-1">Played on {new Date(round.created_at).toLocaleDateString()}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      <section className="text-center">
        ...
        <div className="flex flex-col md:flex-row justify-center gap-4">
          <Link href="/courses" className="px-6 py-3 bg-blue-600 text-white rounded-md hover:bg-blue-700">
            Browse All Courses
          </Link>
          <Link href="/rounds/new" className="px-6 py-3 bg-green-600 text-white rounded-md hover:bg-green-700">
            Start New Round
          </Link>
        </div>
      </section>
